...
[![Oubli Demo](https://img.youtube.com/vi/ITSMAu5MS-s/maxresdefault.jpg)](https://www.youtube.com/watch?v=ITSMAu5MS-s) 
...
**1. Install Oubli**
```bash
pipx install git+https://github.com/ajaygunalan/oubli.git
cd /your/project
oubli setup
```
Restart Claude Code.
...
**Everything is per-project:**
- `.mcp.json` - MCP server registration
- `.claude/` - Hooks, skills, instructions
- `~/.oubli/<project>/` - Your memories and Core Memory (on home filesystem)

Each project gets its own isolated memory. No global state.
...
pipx uninstall oubli
rm -rf ~/.oubli/$(basename $(pwd))/  # Optional: delete this project's memories
```
...
- `/save` - Sweep the conversation and save anything Claude missed
...
Data is stored in `~/.oubli/<project-name>/` under the home directory. Storing under `~` guarantees a POSIX filesystem (ext4 etc.) so LanceDB atomic renames always work, even when the project lives on exFAT/NTFS. Each project has its own isolated memories.

| File | Description |
|------|-------------|
| `~/.oubli/<project>/memories.lance/` | LanceDB database with vector embeddings |
| `~/.oubli/<project>/core_memory.md` | Your Core Memory (human-readable, editable) |
| `~/.oubli/<project>/graph.html` | Memory visualization (generated by `oubli viz`) |

The `OUBLI_DATA_DIR` env var overrides the data directory if set.
...
| Component | Location | Description |
|-----------|----------|-------------|
| Data | `~/.oubli/<project>/` | Memories and Core Memory (created automatically) |
