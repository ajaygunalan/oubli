...
rm -rf .oubli/     # Optional: delete this project's memories
```
...
Data is stored in `.oubli/` inside the project directory (the `cwd` where the MCP server or CLI runs). Each project has its own isolated memories.

| File | Description |
|------|-------------|
| `.oubli/memories.lance/` | LanceDB database with vector embeddings |
| `.oubli/core_memory.md` | Your Core Memory (human-readable, editable) |
| `.oubli/graph.html` | Memory visualization (generated by `oubli viz`) |

Add `.oubli/` to your `.gitignore` — memory data shouldn't be committed.
...
| Component | Location | Description |
|-----------|----------|-------------|
| Data | `.oubli/` | Memories and Core Memory (created automatically) |
